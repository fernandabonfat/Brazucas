...
namespace Brazucas.Controllers
{
    public class BolaoController : Controller
    {
        ...
        {
            JOGOS teste = new JOGOS();
            var jogos = teste.GetJogos(id);
            ViewBag.SELECAO_A = new SelectList(db.SELECOES, "SELECAO_A", "NOMSELE");
            ViewBag.SELECAO_B = new SelectList(db.SELECOES, "SELECAO_B", "NOMSELE"); 


            return View(jogos.Include( j => j.SELECOES).ToList());
        } 
